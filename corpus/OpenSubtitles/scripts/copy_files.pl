...
my $PIERREHOME = '/projects/researchers/researchers01/plison/data/opensubs2016';
my $PIERREHOME = '/projects/researchers/researchers01/plison/data/opensubs2018';

my $DATAHOME = @ARGV ? shift(@ARGV) : 
    '/usit/abel/u1/joerg/work/OPUS/corpus/OpenSubtitles2018';
...
foreach (@languages){
	...
	print STDERR "... copy $_.tar.gz to $lang ...\n";
	system("mkdir -p $DATAHOME/xml/$lang");
	system("cp $PIERREHOME/$_.tar.gz $DATAHOME/xml/$lang/");
    }
    elsif (-e "$PIERREHOME/$lang.tar.gz"){
	print STDERR "... copy $lang.tar.gz to $lang ...\n";
	system("mkdir -p $DATAHOME/xml/$lang");
	system("cp $PIERREHOME/$lang.tar.gz $DATAHOME/xml/$lang/");
    }
    else{
	die "nothing to copy\n";
    }

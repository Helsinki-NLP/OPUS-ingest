...
%DefaultQuery = (DA => '"euro.*"',
		 ...
		 EL => '"e.*"',
		 EN => '"as" [tnt="JJ.*"] "as" <chunk_type="NP"> []+ </chunk>',
		 ES => '"euro.*"',
		 FI => '"euro.*"',
		 FR => '[lem="le"] [pos="NOM"] [pos="PRE.*"] "Parlement"',
		 IT => '[lem="il"] [pos="NOM"] [pos="PRE.*"] "Parlamento"',
		 NL => '"euro.*"',
		 PT => '"euro.*"',
		 SV => '"finns" "det" <c_type="NP.*"> []+ [pos="NC.*"] </c>'
		 );
...
print td({-align => "center"},b("EUROPARL"));
print end_td, 

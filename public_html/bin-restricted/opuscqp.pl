...
#    use WebCqp::Query;
	...
	push (@corpora,map($CorpusName{$_},@{$aligned}));
	foreach (@{$aligned}){
	    if (param("query_$_")){
		my $l=uc($_);
...
#		$cqp.=" :$l ".EncodeString($_,param("query_$_"));
	...
	## special for EDGeS: show verse info
	if ($corpus=~/edges/i){
	    if ($m->{'data'}->{'s_n'}){
		$pos = &FixString($lang,$m->{'data'}->{'s_n'});
		$pos=~s/\s/&nbsp;/g;
	    }
	}
	##-------------------------------------------
...
##    trg ......... name of target language corpus

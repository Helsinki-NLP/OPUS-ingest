...
# my $CWBREG='/Users/joerg/public_html/cwb/reg';
$CWB::Web::Query::Registry = $CWBREG;
...
sub FixString{
    my ($lang,$string)=@_;
    if ($lang=~/^(ar|az|be|bg|bs|fa|he|id|jp|ja|ko|ku|mi|mk|ru|ta|th|uk|vi|xh|zh_tw|zh_cn|zu|bul|chi|jap|jpn|heb|rus)$/){
...
sub EncodeString{
    my ($lang,$string)=@_;
#    $string = decode('utf-8',$string);
    if ($lang=~/^(ar|az|be|bg|bs|he|id|jp|ja|ko|ku|mi|mk|ru|ta|th|uk|vi|xh|zh_tw|zh_cn|zu|bul|chi|jap|jpn|heb|rus)$/){
...
#    $WebCqp::Query::Registry = $corpus;
    ...
    my $query;
#    $CWB::Web::Query::Registry = $CWBREG.'/'.$corpus;

...
#$AttrSID = My::Config::GetAttribute("S_ID");
...
##
sub do_distribution {
  $chapter = My::Config::GetAttribute("CHAPTER_ID");
#  $chapter = My::Config::GetAttribute("SPEAKER_NAME");
  $N = $chapter->max_struc;
  for $i (0 .. $N-1) {
    $count[$i] = 0;				# number of occurrences in this novel
    ($s, $e) = $chapter->struc2cpos($i);
    $rsize[$i] = $e - $s + 1;			# size of novel in tokens
  }
  @matches = $cqp->dump("$id");
  foreach $m (@matches) {
    $struc = $chapter->cpos2struc($m->[0]);	# find novel containing start of match
    $count[$struc]++
      if defined $struc;
  }
  for $i (0 .. $N-1) {
...
#  print header,
  ...
  for $i (0 .. $N-1) {
    ...
    $title = $chapter->struc2str($i);
    $w = int(2 * $ppm[$i] / $aspect + 0.5);
    $h = int($rsize[$i] * sqrt(sqrt($aspect)) / 2000 + 0.5);
#    $h = int($rsize[$i] * sqrt(sqrt($aspect)) / 1000 + 0.5);
    $get_url = My::Forms::MakeGETString([qw<corpus mode query casefold wordlemma sort keyword* context pos xp>],
					{action => 'show', subquery => "<match> [_.CHAPTER_ID=\"$title\" \%l] !"});
#					{action => 'show', subquery => "<match> [_.SPEAKER_NAME=\"$title\" \%l] !"});
    my @alg = CGI::param('alg');
    foreach my $a (@alg){
	$get_url .= ';alg='.$a;
    }
